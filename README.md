...
4. GitHub 저장소 시크릿에 추가: `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`, `APP_STORE_CONNECT_KEY_BASE64`(`.p8` 내용), `MATCH_GIT_URL`, `MATCH_PASSWORD`(직접 정하는 암호화 비밀번호), `APPLE_TEAM_ID`, `MATCH_GIT_BASIC_AUTHORIZATION`(match 저장소 쓰기용 GitHub PAT를 `base64(username:token)` 형식으로 인코딩한 값).
